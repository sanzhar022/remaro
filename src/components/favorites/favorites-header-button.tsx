...
export function FavoritesHeaderButton() {
  ...
  return (
    <Link href="/favorites" title="Избранное" aria-label={hasMounted && count > 0 ? `Избранное, товаров: ${count}` : "Избранное"} className="relative flex min-h-11 min-w-11 flex-col items-center justify-center gap-0.5 rounded-[var(--radius-md)] px-2 text-[11px] text-white/75 transition-colors hover:bg-white/10 hover:text-primary focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-primary/40">
      ...
      {hasMounted && count > 0 && <span className="absolute right-0 top-0 grid min-h-5 min-w-5 place-items-center rounded-full bg-primary px-1 text-[10px] font-bold leading-none text-[var(--navy-950)]">{count > 99 ? "99+" : count}</span>}
    </Link>
  );
}

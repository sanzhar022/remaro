...
export function CartHeaderButton() {
  ...
  return (
    <Link href="/cart" title="Корзина" aria-label={hasMounted && totalItems > 0 ? `Корзина, товаров: ${totalItems}` : "Корзина"} className="relative flex min-h-11 min-w-11 flex-col items-center justify-center gap-0.5 rounded-[var(--radius-md)] px-2 text-[11px] text-muted transition-colors hover:bg-secondary hover:text-primary focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-primary/25">
      ...
      {hasMounted && totalItems > 0 && <span className="absolute right-0 top-0 grid min-h-5 min-w-5 place-items-center rounded-full bg-destructive px-1 text-[10px] font-bold leading-none text-white">{totalItems > 99 ? "99+" : totalItems}</span>}
    </Link>
  );
}

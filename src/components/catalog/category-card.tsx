...
export function CategoryCard({ category }: CategoryCardProps) {
  ...
  return (
    <Link href={`/category/${category.slug}`} className="group flex min-w-0 flex-col overflow-hidden rounded-[var(--radius-lg)] border border-border bg-surface shadow-[var(--shadow-sm)] transition duration-200 hover:-translate-y-0.5 hover:border-primary/50 hover:shadow-[var(--shadow-md)]">
      <div className="relative grid aspect-[16/10] place-items-center overflow-hidden bg-secondary text-[var(--navy-900)] sm:aspect-[5/3]">
